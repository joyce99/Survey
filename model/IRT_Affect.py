...
class IRTAffectNet(nn.Module):
    def __init__(self, user_num, item_num, value_range, a_range, irf_kwargs=None):
        ...
        # 新增：学生情感参数
        self.student_affect = nn.Embedding(self.user_num, 1)
        
        # 题目参数
        self.a = nn.Embedding(self.item_num, 1)  # 区分度
        self.b = nn.Embedding(self.item_num, 1)  # 难度
    ...
    def forward(self, user, item, labels=None):
        ...
        # IRT模型预测概率
        o = self.irf(theta, a, b, None, **self.irf_kwargs)
        
        # 情感处理
        # 将学生情感与题目难度结合
        affect_input = torch.cat([
            stu_affect.squeeze(-1).unsqueeze(1), 
            torch.sigmoid(b).unsqueeze(1)
        ], dim=1)
...
class IRT_Affect:
    def __init__(self, user_num, item_num, value_range=None, a_range=None):
        super(IRT_Affect, self).__init__()
        self.irt_net = IRTAffectNet(user_num, item_num, value_range, a_range)
